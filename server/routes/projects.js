...
router.post("/", protect, authorize("student"), async (req, res) => {
  try {
    ...
    if (!team) {
      return res
        .status(403)
        .json({ message: "Only team leaders can create projects" });
    }
    ...
    if (team.project) {
      return res
        .status(400)
        .json({ message: "Your team already has a project" });
    }
...
router.get("/team", protect, authorize("student"), async (req, res) => {
  try {
    ...
    if (!project) {
      return res
        .status(404)
        .json({ message: "Your team does not have a project yet" });
    }
    ...
    if (req.user.role === "student") {
      ...
      if (!isTeamMember) {
        return res
          .status(403)
          .json({
            message: "Access denied. You are not a member of this project team",
          });
      }
    }
...
router.put("/:id", protect, authorize("student"), async (req, res) => {
  try {
    ...
    if (team.leader.toString() !== req.user._id.toString()) {
      return res
        .status(403)
        .json({ message: "Only team leader can update project details" });
    }
  ...
  async (req, res) => {
    try {
      ...
      if (!isTeamMember) {
        return res
          .status(403)
          .json({ message: "Only team members can add documents" });
      }
